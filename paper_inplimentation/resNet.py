...
class block(nn.Module):
    def __init__(self, in_channels, out_channel, identity_downsample = None, stride = 1):
        super(block, self).__init__()
        self.expansion = 4
        self.conv1 = nn.Conv2d(in_channels, out_channel, kernel_size=1, stride=1, padding=0)
...
class ResNet(nn.Module): #[3,4,6,3]
    ...
    def _make_layer(self, block, num_residual_blocks, out_channel, stride):
        ...
        if stride != 1 or self.in_channels != out_channel * 4:
            identity_downsample = nn.Sequential(nn.Conv2d(self.in_channels, out_channel * 4,  kernel_size=1, 
                                                          stride= stride),
                                                          nn.BatchNorm2d(out_channel *4))
    
        layers.append(block(self.in_channels, out_channel, identity_downsample, stride))
...
def ResNet101(img_channels = 3, num_classes = 1000):
    return ResNet(block, [3, 4, 32, 3], img_channels ,num_classes )
